 ...
 *   isNot: boolean,
 *   utils: {
 *     matcherHint(name: string, received?: string, expected?: string, options?: { isNot?: boolean }): string,
...
// todo: consider breaking matchers into groups per entity (Locator, APIResponse, etc.)
export const matchers = {
  ...
  /**
   * @this {playwright.ExpectMatcherState}
  ...
  async toHaveStatus(response, expected) {
    ...
    const expectationString = `Status of response from url: ${response.url()}\n`
  ...
  async toHaveJsonFields(response, expectedSample) {
    ...
    const actualTree = its.parsed(actual, {
      maxDepth: Infinity,
      maxLeaves: Infinity,
    }).tree
  ...
  async toHaveCountGreaterThanOrEqual(locator, expected, options = {}) {
    ...
    try {
      /*
       * Below we are particularly interested in "expect with retries unless timeout reached".
       ...
       * such functionality:'(.
       * The `toPass` option has not shown any similar issues during our experiments.
      ...
      await playwright
        .expect(async () => {
          const pollingMessage = `expect(${/** @type {any} */ (locator)._selector}).${name}(${expected})`
        ...
        .toPass({
          ...
          timeout: options.timeout ?? playwrightConfig.expect?.timeout ?? 4 * 1000,
        })
    } catch (e) {
      pass = false
    }

    // todo: should we reuse somehow caught error (e), for example, to log it as reason... ?
