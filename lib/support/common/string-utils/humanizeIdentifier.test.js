...
test('capitalizes first letter when explicitly true', () => {
  assert.strictEqual(humanizeIdentifier('testValue', { capitalize: true }), 'Test value')
})

test('does not capitalize first letter when false', () => {
  assert.strictEqual(humanizeIdentifier('testValue', { capitalize: false }), 'test value')
})
...
test('does not capitalize first letter for UPPER_SNAKE_CASE when false', () => {
  assert.strictEqual(humanizeIdentifier('TEST_VALUE', { capitalize: false }), 'test value')
})
...
test('mixed case', () => {
  assert.strictEqual(humanizeIdentifier('mixedCamel_and_snakeCase'), 'Mixed camel and snake case')
})
