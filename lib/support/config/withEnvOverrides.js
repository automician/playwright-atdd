...
const withEnvOverrides = (obj, options = {}) => {
  ...
  return proxy.wrapProperties(obj, (prop, getValue) => {
    ...
    if (typeof originalValue === 'string') {
      return maybeInterpolate(process.env[prop] ?? originalValue)
    }
    ...
    if (typeof originalValue === 'number') {
      return Number(process.env[prop] ?? originalValue)
    }

    if (Array.isArray(originalValue)) {
      return process.env[prop]?.split(',')?.map(it => it.trim()) ?? originalValue
    }
