...
import withYmlOverrides, { WithYmlOverrides } from './withYmlOverrides.js'
...
`)
  const config = withYmlOverrides({ baseUrl: 'https://localhost:3000' }, { file: tempFile })
...
`)
  ...
  const config = withYmlOverrides({ timeout: 3000 }, { file: tempFile })
...
`)
  ...
  const config = withYmlOverrides({ headless: true }, { file: tempFile })
...
`)
  ...
  const config = withYmlOverrides({ browsers: ['safari'] }, { file: tempFile })
...
`)
  ...
  const config = withYmlOverrides({ baseUrl: 'https://localhost' }, { file: tempFile })
...
`)
  ...
  assert.strictEqual(config.baseUrl, 'https://staging.example.com')
  assert.strictEqual(/** @type {any} */ (config).extraProp, undefined)
})
...
`)
  ...
  assert.throws(() => config.timeout, {
    name: 'TypeError',
    message: 'Type mismatch for property "timeout": expected number, got string from YAML',
  })
})
...
`)
  ...
  assert.throws(() => config.headless, {
    name: 'TypeError',
    message: 'Type mismatch for property "headless": expected boolean, got string from YAML',
  })
})
...
`)
  ...
  assert.throws(() => config.browsers, {
    name: 'TypeError',
    message: 'Type mismatch for property "browsers": expected array, got string from YAML',
  })
})
...
`)
  ...
  const withStagingOverrides = WithYmlOverrides.from(tempFile, {
...
`)
  ...
  const withOverrides = WithYmlOverrides.from(tempFile, {
    ignore: ['password'],
  })
...
`)
  ...
  const config = withYmlOverrides({ baseUrl: 'https://localhost' }, { file: tempFile, path: '' })
...
test('interpolates variables in string values', () => {
  writeYaml(`
...
`)
  ...
  const config = withYmlOverrides(
    { baseUrl: 'https://localhost' },
    { file: tempFile, interpolate: { CI_COMMIT_REF_SLUG: 'example-feat-999' } },
  )
...
`)
  ...
  const config = withYmlOverrides(
    { baseUrl: 'https://localhost' },
    { file: tempFile, interpolate: { SUBDOMAIN: 'api', DOMAIN: 'example' } },
  )
...
test('leaves unmatched variables unchanged', () => {
  writeYaml(`
baseUrl: 'https://\${KNOWN}.\${UNKNOWN}.com'
`)

  const config = withYmlOverrides(
    { baseUrl: 'https://localhost' },
    { file: tempFile, interpolate: { KNOWN: 'api' } },
  )

  assert.strictEqual(config.baseUrl, 'https://api.${UNKNOWN}.com')
})

test('interpolate works with nested path', () => {
  writeYaml(`
environments:
  feature-env:
    baseUrl: 'https://feature-\${CI_COMMIT_REF_SLUG}.example.com'
...
test('WithYmlOverrides.from factory supports interpolate option', () => {
  writeYaml(`
environments:
  feature-env:
    baseUrl: 'https://feature-\${CI_COMMIT_REF_SLUG}.example.com'
`)
  ...
  const withFeatureOverrides = WithYmlOverrides.from(tempFile, {
    path: 'environments.feature-env',
    interpolate: { CI_COMMIT_REF_SLUG: 'example-feat-999' },
  })
  const config = withFeatureOverrides({ baseUrl: 'https://localhost' })
...
`)
  ...
  const config = withYmlOverrides(
    { timeout: 3000 },
    { file: tempFile, interpolate: { SOME_VAR: 'value' } },
  )
...
`)
  ...
  const config = withYmlOverrides(
    { baseUrl: 'https://localhost' },
    { file: tempFile, interpolate: {} },
  )

  assert.strictEqual(config.baseUrl, 'https://feature-${CI_COMMIT_REF_SLUG}.example.com')
})
