...
test('should work', () => {
  ...
  assert.deepStrictEqual(withEnvOverrides({ bool_prop: false }), {
    bool_prop: true,
  })

  process.env.bool_prop = 'false'
  assert.deepStrictEqual(withEnvOverrides({ bool_prop: true }), {
    bool_prop: false,
  })
...
test('does not interpolate non-string values', () => {
  ...
  const config = withEnvOverrides({ bool_prop: false }, { interpolate: { SOME_VAR: 'value' } })
